...
impl<'a, A: Arch> Interpreter<'a, A> {
    ...
    pub fn builtin(&mut self, name: String) -> bool {
        ...
        if name == "put_char" {
            print!("{}", self.env[&A::arg_regs()[0]] as u8 as char);
            return true;
        }
...
pub struct BtlInterpreter<'a>{
    table: &'a BtlSymbolTable,
...
impl<'a> BtlInterpreter<'a> {
    pub fn new(table: &'a BtlSymbolTable, main: String) -> Self {
        let mut symbols: HashMap<String, i32> = HashMap::new();
        let mut memory: HashMap<i32, i32> = HashMap::new();
        ...
        for phy in RvArch::callee_saved() {
            env.insert(phy, 0);
        }

        for phy in RvArch::caller_saved() {
            env.insert(phy, 0);
        }
        ...
        for (name, section) in table.symbols.iter() {
            if matches!(section, BtlSection::Text(..)) {
                stats.insert(name.clone(), Statistics::new());
            }
        }
    ...
    pub fn builtin(&mut self, name: String) -> bool {
        if name == "print_i32" {
            print!("{}\n", self.env[&RvArch::arg_regs()[0]]);

            self.write_var(RvArch::ret_reg(), 0);
            return true;
        }
        ...
        if name == "put_char" {
            print!("{}", self.env[&RvArch::arg_regs()[0]] as u8 as char);
            return true;
        }
    ...
    pub fn operation(&mut self, dest: Phys, op: RvOp, args: Vec<i32>) {
        self.write_var(dest, op.eval(args).unwrap());
    }

    pub fn condition(&mut self, cond: RvCond, args: Vec<i32>) -> bool {
        cond.eval(args).unwrap()
    }
    ...
    pub fn interpret_function(&mut self) {
        ...
        //println!("label: {}", self.symbol);

        let mut label = 0;

        'main_loop: loop {
            for bundle in self.cfg().blocks[label].iter() {
                self.stats_mut().bundles += 1;
